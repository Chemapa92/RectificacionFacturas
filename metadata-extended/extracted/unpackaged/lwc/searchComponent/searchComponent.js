...
export default class SearchComponent extends NavigationMixin(LightningElement) {
    ...
    @api fields        = ['Name', 'ShippingStreet', 'Alias__c', 'TaxRate__c'];
    @api where;
    @api displayFields = 'Name, ShippingStreet, Alias__c,toLabel(TaxRate__c), Rating, AccountNumber';
    @api showLabel     = false;
    ...
    handleListHide(event){
        this.template.querySelector('[data-id="menuList"]').classList.remove('slds-is-open');
    }

    handleInputChange(event){
        this.template.querySelector('[data-id="menuList"]').classList.add('slds-is-open');
        ...
        this.delayTimeout = setTimeout(() => {
            //if(searchKey.length >= 2){
                ...
                .then(result => {
                    ...
                    console.log(JSON.stringify(this.searchRecords));
                })
                .catch(error => {
    ...
    handleSelect(event){
        ...
        const attributeChangeEvent = new FlowAttributeChangeEvent(this.objName, this.selectedRecord.Id);
        const attributeChangeEventName = new FlowAttributeChangeEvent(this.objName, this.selectedRecord.Name);
        if(this.isOnFlow) {
            this.dispatchEvent(attributeChangeEvent);
            this.dispatchEvent(attributeChangeEventName);
        } else {
    ...
    handleClose(){
        ...
        const attributeChangeEvent = new FlowAttributeChangeEvent(this.objName, undefined);
        const attributeChangeEventName = new FlowAttributeChangeEvent(this.objName, undefined);
        if(this.isOnFlow) {
            this.dispatchEvent(attributeChangeEvent);
            this.dispatchEvent(attributeChangeEventName);
        } else {
    ...
    validate() {
        if (this.selectedRecord === undefined || this.selectedRecord === null) {
            this.error = 'Necesitas seleccionar la dirección de envío.';
            return {
                isValid: false,
